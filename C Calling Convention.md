...
4. 在从被调用函数返回后（紧跟着call指令的指令）， 需要把超过6个的参数部分从栈上删除。这样栈就恢复到了调用函数之前。
5. 被调用函数的返回值放在了rax 寄存器， caller 可以从这个寄存器或得返回值。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIzNDE3MjMzMSwtNTc1OTY3OTMwLDE2OD
